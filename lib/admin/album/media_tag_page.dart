...
class _MediaTagPageState extends State<MediaTagPage> {
  ...
  Widget build(BuildContext context) {
    ...
    return Scaffold(
      ...
      appBar: AppBar(
        title: GestureDetector(onTap: () => openNewTab("https://drive.google.com/thumbnail?id=${file.id ?? ""}&sz=w1000"), child: Text(file.name ?? "")),
        elevation: 0,
      ),
      body: Container(
                      ...
                      child: CachedNetworkImage(
                        ...
                        errorWidget: (context, _, __) => Center(
                          child: GestureDetector(
                            onTap: () => openNewTab("https://drive.google.com/thumbnail?id=${file.id ?? ""}&sz=w1000"),
                ...
                Expanded(
                  child: StreamBuilder(
                    ...
                    builder: (context, snapshot) {
                      final tags = dbService.medias.where((m) => m.fileId == file.id).toList(growable: true);
                      return MultiTagSelector(
                        initialSelected: tags.map((e) => e.studentId).toList(),
                        onAdd: (studentId) => addStudent(studentId, file.id ?? ""),
                        onRemoved: (studentId) => removeStudent(studentId, file.id ?? ""),
  ...
  Future<void> addStudent(String id, String file) async {
    var m = Media(studentId: id, fileId: file);
    if (dbService.medias.any((m) => widget.files.any((f) => m.fileId == f.id))) {
    } else {
      print("widget.album ${widget.album.name}");
